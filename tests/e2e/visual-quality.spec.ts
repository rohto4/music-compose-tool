...
test('3テーマと主要surfaceをWQHDからスマホまで同じlayoutで表示する', async ({ page }) => {
  ...
  for (const theme of THEMES) {
    ...
    expect(themeMetrics[theme.id]!.chordCount).toBe(14);
  ...
  await page.setViewportSize({ width: 2560, height: 1440 });
  await page.getByRole('button', { name: /^ラフ制作/ }).click();
...
test('ピアノロールをホイールで88鍵範囲へ移動し、初期画面外の低音を配置できる', async ({ page }) => {
  const errors: string[] = [];
  page.on('console', (message) => { if (message.type() === 'error') errors.push(message.text()); });
  page.on('pageerror', (error) => errors.push(error.message));
  ...
  const placedPitch = Number(await page.getByRole('spinbutton', { name: '選択音符の音程' }).inputValue());
  expect(placedPitch).toBeGreaterThanOrEqual(21);
  expect(placedPitch).toBeLessThan(48);
