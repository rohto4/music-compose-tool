...
test('creates and manually saves a humming-first project without console errors', async ({ page }) => {
  ...
  const sections = page.locator('.quick-arrangement-rail article');
  ...
  await expect(sections).toHaveCount(9);
  await expect(sections.last()).toContainText('Bridge');
  await sections.last().getByRole('button', { name: /を前へ/ }).click();
  await expect(sections.nth(7)).toContainText('Bridge');
  const gentleRise = page.locator('.arrangement-template-grid article').filter({ hasText: 'Gentle Rise' });
  await gentleRise.getByRole('button', { name: '現在の流れを置換' }).click();
  await expect(sections).toHaveCount(6);
  await expect(sections.first()).toContainText('Soft Intro');
  const twinDrop = page.locator('.arrangement-template-grid article').filter({ hasText: 'Twin Drop' });
  await twinDrop.getByRole('button', { name: '現在の流れを置換' }).click();
  await expect(sections).toHaveCount(7);
  await page.getByRole('button', { name: '▶ 再生' }).click();
  await expect(page.getByText(/曲全体 · \d+ sec · BAR 1から再生/)).toBeVisible();
