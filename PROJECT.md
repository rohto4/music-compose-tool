...
- 制作surfaceは、コード・音色・展開・AI handoffを組む`ラフ制作`と、既存のDAW-like editorで音符・mixを詰める`カスタマイズ`の2段にする。
...
- local branch: `main`
- remote状態: 2026-07-21の`git ls-remote --symref ... HEAD`ではadvertised HEAD/refなし。localからのpushは未実施。
...
- ラフ制作は、Intro / Verse / Build / Drop / Break / Final Drop向けのKawaii Future Bass伴奏フレーズを4小節単位で挿入できる。kitはコード進行、HOLD / PULSE / SYNC、Bass / Arp / Drum、Pad / Synth、音色を1 commandで同期し、Melody、手編集音、他phraseを保護する。
...
- `音を組む`はコード譜と4小節挿入先を上部、Harmonic Atlas、その下へ挿入可能なコード進行、伴奏phrase、複合FX / Fill、role pattern、音のピースを置く。挿入可能物はclick / tapとdrag & dropを同じProject commandへ接続し、挿入先と情報panelを形・面・accentで区別する。主要sectionは専用handleとkeyboard上下buttonで並べ替え、端末localStorageへ保存し、初期配置へ戻せる。Project音楽dataへlayout設定を混ぜない。
- `ラフ制作`は「曲の設計」「音を組む」「展開を整える」を同時表示せず3つのworkspace tabで使い、`カスタマイズ`は保持したDAW-like editorへ切り替える。
...
- ラフ制作はAI StarterまたはChord Padからコード譜、音色、section、Creative Briefを作る。カスタマイズは同じProjectのDAW-like editorでpitch、timing、length、velocity、track / mixerを扱う。section編集はラフ制作の独自価値として保持する。
